...
import os
import pandas as pd
from recommender.constants import ARTIFACT_DIR, DATA_PREPROCESSING_DIR_NAME, ANIME_FILE_NAME, USER_ANIME_RATINGS_FILE_NAME, USERS_FILE_NAME
...
class _AppStore:
    def __init__(self) -> None:
        ...
        # ── User details CSV ──────────────────────────────────────────────
        self.user_df: pd.DataFrame = pd.read_csv(os.path.join(_PREPROCESSED_DIR, USERS_FILE_NAME))

        # ── Ingestion anime CSV (extras: favorites, watching, completed) ────
        _ing = pd.read_csv(os.path.join(_INGESTION_DIR, "anime.csv"), usecols=["id", "favorites", "watching", "completed"]).rename(columns={"id": "anime_id"})
        # ── Merged detail view ────────────────────────────────────────────
        self._detail_df: pd.DataFrame = self.anime_df.merge(_ing, on="anime_id", how="left")
