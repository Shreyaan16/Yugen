...
from ingestion.entity.artifact_entity import LoadingArtifact
...
class Loading:
    ...
    def _upload_blob(self, blob_name: str, data: str) -> None:
        blob_client = self._blob_service.get_blob_client(container=self.config.azure_container_name, blob=blob_name)
    ...
    def export_all_tables(self) -> list[str]:
        self._ensure_connected()
        target_schema = self.config.schema
